...
// Get the pointer to the first indicator
uint8_t *pIndicators = (uint8_t *)&eeprom_ec_config.ind1;
...
bool indicators_callback(void) {
    ...
    for (index = 0; index < INDICATOR_NUMBER; index++) {
        ...
        if (set_indicator(*(current_indicator_p)))
            ...
            sethsv(0, 0, 0, (LED_TYPE *)&led[current_indicator_p->index]);
    }
    return true;
}
